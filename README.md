...
The aim is to build and deploy machine learning (and deep learning) models with a focus on the whole pipeline (cleaning, training, serving layer, Docker container, CI/CD, deploy and monitor), not just the traditional approach of doing everything in a notebook.
...
| FastAPI | HTTP prediction endpoint |
| Docker | Containerization |
| MLflow | ML experiment tracking + Registry |
| GitHub Actions | CI/CD — build, test, deploy |
| AWS ECS (Fargate) + ALB | Serverless container orchestration and load balancing |
